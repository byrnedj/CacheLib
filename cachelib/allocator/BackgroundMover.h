...
struct BackgroundMoverAPIWrapper {
  ...
  // @param tid               the tier id to traverse
  ...
  static double getApproxUsage(C& cache, unsigned int tid, unsigned int pid, unsigned int cid) {
...
class BackgroundMover : public PeriodicWorker {
 public:
  ...
  BackgroundMover(Cache& cache, size_t evictionBatch, size_t promotionBatch, double targetFree);
...
BackgroundMover<CacheT>::BackgroundMover(
    Cache& cache, size_t evictionBatch, size_t promotionBatch, double targetFree)
    : cache_(cache), evictionBatch_(evictionBatch), promotionBatch_(promotionBatch),
...
void BackgroundMover<CacheT>::checkAndRun() {
  while (true) {
    ...
    for (auto md : assignedMemory) {
      ...
      if (classUsage < (1-targetFree_)) {
        // no eviction work to be done since there is already at least targetFree
        // remaining in the class
        evictionBatch = 0;
      }  
      size_t promotionBatch = tid == numTiers_ - 1 ? 0 : promotionBatch_;
...
BackgroundMoverStats BackgroundMover<CacheT>::getStats() const noexcept {
  ...
  stats.avgItemsMoved = (double) (stats.numEvictedItems + stats.numPromotedItems) / (double) numTraversals_;
  stats.lastTraversalTimeNs = traversalStats_.getLastTraversalTimeNs();
  stats.avgTraversalTimeNs = traversalStats_.getAvgTraversalTimeNs(numTraversals_);
