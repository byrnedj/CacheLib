...
//
// In MMLru3, we have 3 LRU queues. Items are shared among these 3 queues randomly 
...
class MMLru3 {
 public:
  ...
  // This is not applicable for MMLru3, just for compile of cache allocator
  enum LruType { Lru1, Lru2, Lru3, NumTypes };

  // Config class for MMLru3
  struct Config {
  ...
  struct Container {
   ...
   private:
    ...
    void markLRU2(T& node) noexcept {
      node.template setFlag<RefFlags::kMMFlag1>(); //lruMutex[2]
    }
    ...
    void markLRU3(T& node) noexcept {
      node.template setFlag<RefFlags::kMMFlag2>(); //lruMutex[3]
    }

    void unmarkLRU3(T& node) noexcept {
      node.template unSetFlag<RefFlags::kMMFlag2>();
    }

    bool isLRU3(const T& node) const noexcept {
    ...
    uint8_t getLRUNum(T& node) {
      ...
      if (isLRU3(node)) return LruType::Lru3;
...
template <typename T, MMLru3::Hook<T> T::*HookPtr>
MMLru3::Container<T, HookPtr>::Container(serialization::MMLru3Object object,
...
template <typename T, MMLru3::Hook<T> T::*HookPtr>
bool MMLru3::Container<T, HookPtr>::recordAccess(T& node,
...
template <typename T, MMLru3::Hook<T> T::*HookPtr>
cachelib::EvictionAgeStat MMLru3::Container<T, HookPtr>::getEvictionAgeStat(
    uint64_t projectedLength) const noexcept {
  return EvictionAgeStat{};
}

template <typename T, MMLru3::Hook<T> T::*HookPtr>
void MMLru3::Container<T, HookPtr>::setConfig(const Config& newConfig) {
...
template <typename T, MMLru3::Hook<T> T::*HookPtr>
typename MMLru3::Config MMLru3::Container<T, HookPtr>::getConfig() const {
  return lruMutex_[LruType::NumTypes]->lock_combine([this]() { return config_; });
}


template <typename T, MMLru3::Hook<T> T::*HookPtr>
bool MMLru3::Container<T, HookPtr>::add(T& node) noexcept {
...
template <typename T, MMLru3::Hook<T> T::*HookPtr>
void MMLru3::Container<T, HookPtr>::addNodeLocked(T& node, const Time& currTime, const uint8_t num) {
...
template <typename T, MMLru3::Hook<T> T::*HookPtr>
template <typename It>
uint32_t MMLru3::Container<T, HookPtr>::addBatch(It begin, It end) noexcept {
...
template <typename T, MMLru3::Hook<T> T::*HookPtr>
typename MMLru3::Container<T, HookPtr>::LockedIterator
MMLru3::Container<T, HookPtr>::getEvictionIterator() const noexcept {
...
template <typename T, MMLru3::Hook<T> T::*HookPtr>
template <typename F>
void MMLru3::Container<T, HookPtr>::withEvictionIterator(F&& fun) {
...
template <typename F>
void
MMLru3::Container<T, HookPtr>::withPromotionIterator(F&& fun) {
  // same as eviction
  auto num = (folly::Random::rand32() % LruType::NumTypes);
  auto lck = LockHolder{*lruMutex_[num], std::try_to_lock};
  while (!lck.owns_lock()) {
...
template <typename T, MMLru3::Hook<T> T::*HookPtr>
template <typename F>
void MMLru3::Container<T, HookPtr>::withContainerLock(F&& fun) {
  std::lock(*lruMutex_[LruType::Lru1], *lruMutex_[LruType::Lru2], *lruMutex_[LruType::Lru3]);
  LockHolder l1(*lruMutex_[LruType::Lru1], std::defer_lock);
  LockHolder l2(*lruMutex_[LruType::Lru2], std::defer_lock);
  LockHolder l3(*lruMutex_[LruType::Lru3], std::defer_lock);
  fun();
}

template <typename T, MMLru3::Hook<T> T::*HookPtr>
void MMLru3::Container<T, HookPtr>::removeLocked(T& node, const uint8_t num) {
...
template <typename T, MMLru3::Hook<T> T::*HookPtr>
bool MMLru3::Container<T, HookPtr>::remove(T& node) noexcept {
  auto num = getLRUNum(node);
...
template <typename T, MMLru3::Hook<T> T::*HookPtr>
void MMLru3::Container<T, HookPtr>::remove(Iterator& it) noexcept {
  T& node = *it;
  XDCHECK(node.isInMMContainer());
  ++it;
...
template <typename T, MMLru3::Hook<T> T::*HookPtr>
bool MMLru3::Container<T, HookPtr>::replace(T& oldNode, T& newNode) noexcept {
  ...
  return lruMutex_[num]->lock_combine([this, &oldNode, &newNode, num]() {
    ...
    switch (num) {
      ...
      case LruType::Lru2:
        ...
        break;
      case LruType::Lru3:
        if (!isLRU3(oldNode)) return false;
        unmarkLRU3(oldNode);
        markLRU3(newNode);
        break;
    }
...
template <typename T, MMLru3::Hook<T> T::*HookPtr>
serialization::MMLru3Object MMLru3::Container<T, HookPtr>::saveState()
    const noexcept {
...
template <typename T, MMLru3::Hook<T> T::*HookPtr>
MMContainerStat MMLru3::Container<T, HookPtr>::getStats() const noexcept {
...
template <typename T, MMLru3::Hook<T> T::*HookPtr>
MMLru3::Container<T, HookPtr>::LockedIterator::LockedIterator(
