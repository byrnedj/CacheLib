...
//
// In MMLru2, we have 2 LRU queues. Items are shared among these 2 queues (among
// the threads) for now. This allows us to have one thread inserting and another
// thread evicting from the class at the same time.
class MMLru2 {
 public:
  ...
  using SerializationType = serialization::MMLru2Object;
  using SerializationConfigType = serialization::MMLruConfig;
  using SerializationTypeContainer = serialization::MMLru2Collection;
  ...
  struct Container {
   private:
    ...
    using CompressedPtr = typename T::CompressedPtrType;
   ...
   public:
    ...
    Container(serialization::MMLru2Object object, PtrCompressor compressor);
    ...
    class LockedIterator : public Iterator {
     ...
     private:
      // private because it's easy to misuse and cause deadlock for MMLru2
    ...
    //
    serialization::MMLru2Object saveState() const noexcept;
   ...
   private:
    ...
    // Config for this lru.
    // Write access to the MMLru2 Config is serialized.
    // Reads may be racy.
    Config config_{};
...
MMLru2::Container<T, HookPtr>::Container(serialization::MMLru2Object object,
...
typename MMLru2::Config MMLru2::Container<T, HookPtr>::getConfig() const {
  return lruMutex_[LruType::NumTypes]->lock_combine(
      [this]() { return config_; });
}
...
bool MMLru2::Container<T, HookPtr>::add(T& node) noexcept {
  ...
  /*
  num = folly::Random::rand32() % LruType::NumTypes; //or we can use a random
  number auto lck = LockHolder{*lruMutex_[num], std::try_to_lock}; while
  (!lck.owns_lock()) { num = (num + 1) % LruType::NumTypes; lck =
  LockHolder{*lruMutex_[num], std::try_to_lock};
  }
  if (node.isInMMContainer() ||
...
template <typename T, MMLru2::Hook<T> T::*HookPtr>
void MMLru2::Container<T, HookPtr>::addNodeLocked(T& node,
                                                  const Time& currTime,
                                                  const uint8_t num) {
...
uint32_t MMLru2::Container<T, HookPtr>::addBatch(It begin, It end) noexcept {
  ...
  // auto num = folly::Random::rand32() % LruType::NumTypes; //or we can use a
  // random number
...
template <typename T, MMLru2::Hook<T> T::*HookPtr>
template <typename F>
void MMLru2::Container<T, HookPtr>::withPromotionIterator(F&& fun) {
  /*
  // same as eviction
  auto num = (folly::Random::rand32() % LruType::NumTypes);
  auto lck = LockHolder{*lruMutex_[num], std::try_to_lock};
  ...
  */
  ...
  if (config_.useCombinedLockForIterators) {
    lruMutex_[num]->lock_combine(
        [this, num, &fun]() { fun(Iterator{lru_.getList(num).begin()}); });
  } else {
    LockHolder l(*lruMutex_[num]);
    fun(Iterator{lru_.getList(num).begin()});
  }
...
template <typename T, MMLru2::Hook<T> T::*HookPtr>
serialization::MMLru2Object MMLru2::Container<T, HookPtr>::saveState()
    const noexcept {
  ...
  *configObject.tryLockUpdate() = config_.tryLockUpdate;
  ...
  serialization::MMLru2Object object;
...
MMContainerStat MMLru2::Container<T, HookPtr>::getStats() const noexcept {
  auto stat = lruMutex_[LruType::NumTypes]->lock_combine([this]() {
    ...
    return folly::make_array(
        size(), 0, lruRefreshTime_.load(std::memory_order_relaxed));
  });
