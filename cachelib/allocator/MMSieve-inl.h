...
uint32_t MMSieve::Container<T, HookPtr>::addBatch(It begin, It end) noexcept {
  uint32_t i = 0;
...
void MMSieve::Container<T, HookPtr>::withEvictionIterator(F&& fun) {
  ...
  //  lruMutex_->lock_combine([this, &fun]() { fun(LockedIterator{fifo_.rbegin()}); });
  //} else {
  //  LockHolder lck{*lruMutex_};
  //}
  fun(LockedIterator{&fifo_});
}

template <typename T, MMSieve::Hook<T> T::*HookPtr>
template <typename F>
void MMSieve::Container<T, HookPtr>::withPromotionIterator(F&& fun) {
  //if (config_.useCombinedLockForIterators) {
  //  lruMutex_->lock_combine([this, &fun]() { fun(LockedIterator{fifo_.begin()}); });
  //} else {
  //  LockHolder lck{*lruMutex_};
  //}
  fun(LockedIterator{&fifo_});
