...
exp_res_file_lat = "/tmp/exp3_lat"
exp_res_file_tp = "/tmp/exp3_tp"

with open(exp_res_file_lat, 'w') as f:
    line = "size,ratio,bg-int,bg-size,mem,op,percentile,value\n"
    f.write(line)
with open(exp_res_file_tp, 'w') as f:
    line = "size,ratio,bg-int,bg-size,mem,variable,value\n"
    f.write(line)
                ...
                for bi in backgroundEvictorInterval:
                    for keepfree in backgroundEvictorKeepFree:
                        for m in memconfig:
                            ...
                            exp_conf = prefix + "/config_wrkld_" + str(l) + "_size_" + str(s) + "_ratio_" + str(r) + "_reduced-cs_" + "_mem_" + m + "_bgi_" + str(bi) + "_keepfree_" + str(keepfree)
                            ...
                            with open(exp_res_file_tp, 'ab') as f:
                                line = str(s) + "," + str(r) + "," + str(bi) + "," + str(keepfree) + "," + m + ",get," + str(tp[0]) + '\n'
                                f.write(line)
                                line = str(s) + "," + str(r) + "," + str(bi) + "," + str(keepfree) + "," + m + ",set," + str(tp[1]) + '\n'
                                f.write(line)
                                line = str(s) + "," + str(r) + "," + str(bi) + "," + str(keepfree) + "," + m + ",hr," + str(hit_ratio) + '\n'
                                f.write(line)
                            ...
                            with open(exp_res_file_lat, 'ab') as f:
                                for l in latency:
                                    p = l.split(',')
                                    if (len(p) == 3):
                                        line = str(s) + "," + str(r) + "," + str(bi) + "," + str(keepfree) + "," + m + "," + p[0] + "," + p[1] + "," + str(p[2]) + '\n'
                                        f.write(line)
