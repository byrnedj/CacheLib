...
struct BinaryRequest {
  ...
  BinaryRequest(uint8_t keySize, size_t valueSize, uint16_t repeats,
                 uint8_t op, uint16_t ttl, size_t keyOffset)
      : keySize_(keySize), valueSize_(valueSize), repeats_(repeats),
        op_(op), ttl_(ttl), keyOffset_(keyOffset) {}

  std::string_view getKey(char* baseAddr) const {
    return std::string_view(reinterpret_cast<const char*>(baseAddr + keyOffset_),keySize_);
  }
};
...
struct Request {
  ...
  Request(std::string_view k,
          size_t* b,
          OpType o,
          uint64_t reqId)
      : key(k), sizeBegin(b), requestId(reqId), op(o) {}
  ...
  inline void update(std::string_view &k, size_t *valueSize, OpType o,
              uint16_t ttl, uint64_t reqId) {
      key = k;
      op = o;
      ttlSecs = ttl;
      requestId = reqId;
